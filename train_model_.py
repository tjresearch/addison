...
def ResNet50(include_top=False, load_weight=True, weights='imagenet',
    ...
    if include_top:
        ...
        # tensor length: SS * (C +B(5) ) : 968
        x = Dense(11*11*(3+1*5), activation='linear', name='yolo_clf_3')(x)
...
def loop_body(t_true, t_pred, i, ta):
    ...
    # Arguments
    t_true: the ground truth tensor; shape: (batch_size, 4477)
    t_pred: the predicted tensor; shape: (batch_size, 4477)
...
def generator(label_keys, label_frames, batch_size=32, folder='cleaned_imgs_v3/'):
    '''
    Generator function
    # Arguments
...
if __name__ == '__main__':
    ...
    with open('label_frames_logosIW3.p', 'rb') as f:
        label_frames = pickle.load(f)

    label_keys = list(label_frames.keys())
    print(label_keys[0])
    print(label_frames[label_keys[0]])
    ...
    history = model.fit_generator(train_generator, validation_data=validation_generator,
                                    steps_per_epoch=len(lbl_train)//32, epochs=15,
                                    validation_steps=len(lbl_validn)//32,
